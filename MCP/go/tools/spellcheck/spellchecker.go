...
func SpellcheckerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["ActionType"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("ActionType=%v", val))
		}
		if val, ok := args["AppName"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("AppName=%v", val))
		}
		if val, ok := args["cc"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("cc=%v", val))
		}
		if val, ok := args["ClientMachineName"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("ClientMachineName=%v", val))
		}
		if val, ok := args["DocId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("DocId=%v", val))
		}
		...
		if val, ok := args["PreContextText"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("PreContextText=%v", val))
		}
		if val, ok := args["PostContextText"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("PostContextText=%v", val))
		}
...
func CreateSpellcheckerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_spellcheck",
		...
		mcp.WithString("ActionType", mcp.Description("A string that's used by logging to determine whether the request is coming from an interactive session or a page load. The following are the possible values. 1) Edit—The request is from an interactive session 2) Load—The request is from a page load")),
		mcp.WithString("AppName", mcp.Description("The unique name of your app. The name must be known by Bing. Do not include this parameter unless you have previously contacted Bing to get a unique app name. To get a unique name, contact your Bing Business Development manager.")),
		...
		mcp.WithString("ClientMachineName", mcp.Description("A unique name of the device that the request is being made from. Generate a unique value for each device (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections.")),
		mcp.WithString("DocId", mcp.Description("A unique ID that identifies the document that the text belongs to. Generate a unique value for each document (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections.")),
		...
		mcp.WithString("SessionId", mcp.Description("A unique ID that identifies this user session. Generate a unique value for each user session (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections")),
